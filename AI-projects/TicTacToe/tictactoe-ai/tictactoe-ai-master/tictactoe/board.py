...
class Board:
    ...
    def wild_resolve(self) -> tuple[Optional["Symbol"], Optional[int]]:
        #check the new wild rule
        #first check for 3 in a row
        #if yes, check if opponent can make 3 in a row
        #if yes again, losing position
        position = self.get_connection()
        #if not 3 in a row, do nothing
        if not position:
            return (None, None)
        
        #make sure it is symbol 3 in a row
        connection_symbol = self.square_value(position[0])
        opponent_symbol = Symbol.CROSS if connection_symbol == Symbol.CIRCLE else Symbol.CIRCLE
        
        #check opponent position and possible moves
        for i in list(self.empty_squares):
            self.push(i, opponent_symbol)
            #init opp_position to false
            opp_position = False
            #brute force check for opponent win, get_connection() doesnt work
            for a, b, c in self.win_conditions:
                sa = self.square_value(a)
                sb = self.square_value(b)
                sc = self.square_value(c)
                if sa == opponent_symbol and sb == opponent_symbol and sc == opponent_symbol:
                    opp_position = True
                    break
            self.undo(i)

            if opp_position:
                return (opponent_symbol, i) #opponent counter win
        return (connection_symbol, None)
