...
class Gui:
    ...
    def display_status(self):
        #check rebuttal status of board
        if not self.board.get_connection() and not self.board.is_draw():
            self.rebuttal = False

        #check winner
        #allow fo rebuttal move before declare winner
        winner = self.board.winner()
        if (winner is not None) and (not self.rebuttal):
            winner_sym, rebuttal_sq = self.board.wild_resolve()
            if rebuttal_sq is not None:
                self.board.push(rebuttal_sq, winner_sym)
            self.rebuttal = True
