...
def ucs(start, model="A"):
    #check if valid state
    if not valid(start):
        raise ValueError("Invalid Start State")
    fringe = [(0,Node(start, None, 0))]
    #ucs uses list with sorted weights [weight, node]
    ...
    while fringe:
        ...
        index = 0
        min_cost, min_node = fringe[0]
        for i in range(1, len(fringe)):
            cost_i, node_i = fringe[i]
            if cost_i < min_cost:
                index=i
                min_cost, min_node = cost_i, node_i

        #get least cost node
        cost, node = fringe.pop(index)
        s = node.state

        #check if current least cost path to node
        if cost != visited_cost.get(s, float("inf")):
            continue
        ...
        for nxt in successors(s):
            ...
            step = edge_cost(s, nxt, model)
            new_cost = cost + step
            if new_cost < visited_cost.get(nxt, float("inf")):
                visited_cost[nxt] = new_cost
                fringe.append((new_cost, Node(nxt, node, node.depth + 1)))
...
def main():
    ...
    model = "A"
    #run ucs A
    path_A, cost_A, exp_A = ucs(start, model=model)
    print(f"The solution of Q2.1.a (UCS, cost model {model}) is:")
    if path_A:
    ...
    model = "B"
    #run ucs B
    path_B, cost_B, exp_B = ucs(start, model=model)
    print(f"The solution of Q2.1.b (UCS, cost model {model}) is:")
    if path_B:
